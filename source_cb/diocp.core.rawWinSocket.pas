...
begin
  ...
  Result := diocp.winapiWinsock2.Bind(FSocketHandle, TSockAddr(sockaddr), SizeOf(sockaddr)) = 0;
end;
...
begin
  ...
  begin
    ...
    diocp.winapiWinsock2.shutdown(lvTempSocket, SD_BOTH);
...
begin
  ...
  Result := diocp.winapiWinsock2.Connect(FSocketHandle, TSockAddr(sockaddr), sizeof(TSockAddrIn))  = 0;
end;
...
begin
  ...
  lvRet := diocp.winapiWinsock2.Connect(FSocketHandle, TSockAddr(sockaddr), sizeof(TSockAddrIn));
  if lvRet = 0 then
  begin  // 连接成功
    lvFlags := 0;  // 非阻塞模式
    ioctlsocket(FSocketHandle, FIONBIO, lvFlags);
  ...
  begin
    ...
    lvRet := diocp.winapiWinsock2.select(FSocketHandle, nil, @fs, nil, Timeptr);
...
begin
  ...
  Result := diocp.winapiWinsock2.Listen(FSocketHandle, queueSize) = 0;
end;

function TRawSocket.PeekBuf(var data; const len: Integer): Integer;
begin
  Result := diocp.winapiWinsock2.recv(FSocketHandle, data, len, MSG_PEEK);
end;
...
begin
  Result := diocp.winapiWinsock2.recv(FSocketHandle, data, len, 0);
end;
...
begin
  ...
  Result := diocp.winapiWinsock2.select(FSocketHandle + 1, ReadFdsptr, WriteFdsptr, ExceptFdsptr, Timeptr);
...
begin
  Result := diocp.winapiWinsock2.Send(FSocketHandle, data, len, 0);
end;
...
begin
  Result := diocp.winapiWinsock2.shutdown(FSocketHandle, pvHow);
end;
